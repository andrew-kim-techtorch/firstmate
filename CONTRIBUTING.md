...
tests/fm-pr-body-check.test.sh            # fm-pr-body-check.sh screenshot-render checks: clean body passes, local paths (/var/folders, /private/tmp, /Users/...) and raw.githubusercontent.com fail, --ui requires an inline GitHub attachment (blob links do not satisfy it), malformed URLs fail fast, and a gh fetch failure fails loudly
