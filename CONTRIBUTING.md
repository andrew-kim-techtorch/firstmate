...
tests/fm-pr-body-check.test.sh            # fm-pr-body-check.sh screenshot-render checks: clean body passes, local paths (/var/folders, /private/tmp, /Users/...) and raw.githubusercontent.com fail, --ui requires an inline GitHub attachment (blob links do not satisfy it), malformed URLs fail fast, a gh fetch failure fails loudly, a body with a #N reference passes without warning, and a body with no thread link exits 0 with a warning
