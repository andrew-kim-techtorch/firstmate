...
tests/fm-lock.test.sh                     # fm-lock.sh session lock: recycled-pid identity rejection, live-identity hold, dead-pid stale, old single-line lock degrading to pid+comm-only liveness, and the status recycled-pid report
...
tests/fm-usage.test.sh                    # fm-usage.sh per-task token+cost audit: joins state/*.meta worktree= to a stubbed codeburn export for cost/summed-tokens/PR, zero-usage project reports zeroes, meta without worktree= skipped with a warning, and an empty state dir reports cleanly
...
tests/fm-pr-check.test.sh                 # fm-pr-check.sh wires the canonical body gate in: a bare body and a pipeline-default body fail without recording pr= or arming the merge poll, a canonical body passes and records pr= and arms the poll, FM_PR_CHECK_SKIP_BODY_GATE=1 bypasses the gate, an otherwise-canonical body with a local evidence path is auto-sanitized and retried so pr= is recorded and the poll armed, and a body that still fails other substance checks after auto-sanitize stays unarmed
tests/fm-pr-body-sanitize.test.sh         # fm-pr-body-sanitize.sh: /var/folders, /private/tmp, and /Users/....png references are each replaced and the PR edited, a clean body is a no-op (gh-axi never called), a malformed PR URL fails fast, and a sanitized body clears fm-pr-body-check.sh's local-path gate
tests/fm-pr-body-check.test.sh            # fm-pr-body-check.sh substance checks: a full canonical body passes, a bare/pipeline-default body and one missing the mermaid schematic, a table, or with vague Evidence fail, an unsafe mermaid diagram (unquoted specials in a label, a literal \n, a reserved keyword node id) fails while a fully-quoted cylinder-shape diagram, the renamed keyword id, a diagram-type header line, and a subgraph opener do not false-positive, a present-but-broken mmdc parse-errors hard-fail but mmdc infra/launch/timeout failures downgrade to an advisory warning and pass and the real-parser gate falls back to npx when mmdc is absent, local paths (/var/folders, /private/tmp, /Users/...) and raw.githubusercontent.com fail, --ui requires an inline GitHub attachment and a Before/After table (blob links do not satisfy it), malformed URLs fail fast, a gh fetch failure fails loudly, a #N reference passes without warning, and a body with no thread link exits 0 with a warning
tests/fm-crew-state.test.sh               # fm-crew-state.sh current-state reconciliation: run-step authority including closed panes and ci log-tail checks-green detection, stale checks-green and needs-decision/blocked superseded by resumed work, genuine-parked, cross-branch runs-list attribution, pane/status-log fallback, scout skip, torn-down/missing-meta graceful, and usage-blocked detection (pure signature, pane dialog with reset time, failed-run override, repeated exit-status-1 crashes, and not-a-false-positive on a bare build failure)
tests/fm-usage-park.test.sh               # fm-usage-park.sh park-don't-relaunch recovery: parked-list/numeric-epoch/executable shared check.sh artifacts, the check fires once + self-removes after the reset, multiple parks share ONE check converging on the earliest window, an estimated fallback epoch when no reset time is known, and a leading-zero reset minute is not read as octal
