...
tests/fm-pr-check.test.sh                 # fm-pr-check.sh wires the canonical body gate in: a bare body and a pipeline-default body fail without recording pr= or arming the merge poll, a canonical body passes and records pr= and arms the poll, and FM_PR_CHECK_SKIP_BODY_GATE=1 bypasses the gate
