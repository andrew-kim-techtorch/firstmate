...
#                 "GATE_HOOK: <project>: needs repair|patched|patch failed ...".
...
#          A GATE_HOOK line means a project's no-mistakes post-receive gate hook
#          regressed to the buggy bare "$(pwd)" gate-path pattern (upstream bug,
#          regenerated by `no-mistakes init`/upgrade); it is repaired to
#          "$(/bin/pwd -P)" automatically when this session holds the fleet lock, or
#          reported for repair when read-only (fm-gate-hook-check.sh).
...
# Gate-hook self-heal: repair any project's no-mistakes gate hook stuck on the
# buggy bare "$(pwd)" pattern (fm-gate-hook-lib.sh); read-only sessions only
# report it, since repairing is a write and the mutating sweeps below are
# gated on holding the fleet lock the same way.
if [ -x "$FM_ROOT/bin/fm-gate-hook-check.sh" ]; then
  if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" = 1 ]; then
    FM_ROOT_OVERRIDE="$FM_ROOT" FM_HOME="$FM_HOME" FM_PROJECTS_OVERRIDE="$PROJECTS" \
      "$FM_ROOT/bin/fm-gate-hook-check.sh" --detect-only
  else
    FM_ROOT_OVERRIDE="$FM_ROOT" FM_HOME="$FM_HOME" FM_PROJECTS_OVERRIDE="$PROJECTS" \
      "$FM_ROOT/bin/fm-gate-hook-check.sh"
  fi
fi
